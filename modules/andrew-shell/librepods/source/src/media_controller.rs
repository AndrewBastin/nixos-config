    ...
    async fn playback_listener_loop(
        ...
        loop {
            ...
            if !was_playing && is_playing {
                // Snapshot the AACP state under the lock, then drop it before
                // calling send_*. send_packet (aacp.rs:441) acquires the same
                // state lock; holding it across the send_* calls below would
                // self-deadlock the Tokio Mutex (not reentrant). Upstream had
                // this hang the moment connected_devices contained any peer.
                let (in_ear, connected_devices) = {
