...
pub struct AACPManagerState {
    ...
    /// Cache of peer MAC -> human-readable name, populated from SmartRouting
    /// plist messages (btAddress + btName fields).
...
/// Extract a string value from a SmartRouting plist payload by key.
/// The wire format encodes each string as `<marker><bytes>` where
/// `marker = 0x40 + len` for short strings (observed lengths up to 17 for
/// MAC addresses). Returns the value following the named key.
fn extract_smart_routing_value<'a>(payload: &'a [u8], key: &[u8]) -> Option<&'a str> {
    let key_marker = 0x40_u8.checked_add(key.len() as u8)?;
    let mut i = 0usize;
    while i + 1 + key.len() < payload.len() {
        if payload[i] == key_marker && &payload[i + 1..i + 1 + key.len()] == key {
            let value_marker_pos = i + 1 + key.len();
            if value_marker_pos >= payload.len() {
                return None;
            }
            let value_marker = payload[value_marker_pos];
            // Length markers observed in the wild: 0x40..=0x7F (length up to 63).
            if !(0x40..=0x7F).contains(&value_marker) {
                return None;
            }
            let value_len = (value_marker - 0x40) as usize;
            let value_start = value_marker_pos + 1;
            let value_end = value_start + value_len;
            if value_end > payload.len() {
                return None;
            }
            return std::str::from_utf8(&payload[value_start..value_end]).ok();
        }
        i += 1;
    }
    None
    ...
    pub async fn receive_packet(&self, packet: &[u8]) {
        ...
        match opcode {
            ...
            opcodes::CONNECTED_DEVICES => {
                // Payload layout (after the 4-byte HEADER + 2-byte opcode):
                //   payload[0..2] = opcode echo (2e 00)
                //   payload[2]    = sub-opcode (observed: 0x01 for full list)
                //   payload[3]    = reserved (observed: 0x00)
                //   payload[4]    = count of device entries
                //   payload[5..]  = count * 8 bytes (6-byte MAC + info1 + info2)
                // Upstream had `count = payload[2]`, which silently aliased the
                // sub-opcode byte as count and only ever exposed device 0 to the
                // event channel. Fixed to read from payload[4].
                if payload.len() < 5 {
                    error!(
            ...
            opcodes::SMART_ROUTING_RESP => {
                ...
                // Cache peer MAC -> name mappings from any plist that carries
                // both btAddress and btName so the UI/notification layer can
                // show "iPhone" instead of a raw MAC.
                let bt_addr = extract_smart_routing_value(&payload[2..], b"btAddress");
                let bt_name = extract_smart_routing_value(&payload[2..], b"btName");
                if let (Some(addr), Some(name)) = (bt_addr, bt_name) {
                    if !addr.is_empty() && !name.is_empty() {
                        let mut state = self.state.lock().await;
                        state
                            .peer_names
                            .insert(addr.to_string(), name.to_string());
                        debug!("Cached peer name: {} -> {}", addr, name);
                    }
                }
